...
namespace sf::priv
{
class WindowImplHarmony : public WindowImpl
{
public:
    explicit WindowImplHarmony(WindowHandle handle);
    WindowImplHarmony(VideoMode mode, const String& title, std::uint32_t style, State state, const ContextSettings& settings);
    ~WindowImplHarmony() override;
...
private:
    void initialize(WindowHandle requestedHandle);
