...
struct SocketSelector::SocketSelectorImpl
{
             ...
             std::function<void(ReadinessType readinessType)> readyCallback)
    {
        ...
        if (readinessType & Receive)
            event.events |= readMask;
        if (readinessType & Send)
            ...
            iter != sockets.end())
        {
            ...
            // Check if we just have to update the callback
    ...
    void dispatchReadyCallbacks()
    {
        for (auto i = 0; i < eventsCount; ++i)
        {
            ...
            ReadinessType readinessType{};
            if ((event.events & readMask) != 0)
                readinessType |= Receive;
            if ((event.events & writeMask) != 0)
                readinessType |= Send;
            if (readinessType)
                callback(readinessType);
        }
    }
    ...
    void dispatchReadyCallbacks()
    {
        for (auto i = 0; i < eventsCount; ++i)
        {
            ...
            // called once with Receive and once with Send
    ...
    bool isReady(const Socket* socket, ReadinessType readinessType) const
    {
        ...
        auto result = false;
        if (readinessType & Receive)
            result |= (FD_ISSET(handle, &socketsReadable) != 0);
        if (readinessType & Send)
            result |= (FD_ISSET(handle, &socketsWritable) != 0);
        return result;
    }
    ...
    void dispatchReadyCallbacks()
    {
        for (const auto& socket : sockets)
        {
            ...
            ReadinessType readinessType{};
            if (FD_ISSET(socket.handle, &socketsReadable) != 0)
                readinessType |= Receive;
            if (FD_ISSET(socket.handle, &socketsWritable) != 0)
                readinessType |= Send;
            if (readinessType)
                callback(readinessType);
        }
